...
class Creator extends React.Component {
	...
	handleTitleSubmit(event) {
		...
		this.setState({iCanHazPopup: false})
	...
	handleBlur(event) {
		if (event.target.value <= 0) {
			qset.randomization = event.target.value = 0
		} else if (event.target.value > (qset.dimensions.x * qset.dimensions.y)) {
			qset.randomization = event.target.value = qset.dimensions.x * qset.dimensions.y
		} else {
			qset.randomization = event.target.value
		}
		event.preventDefault()
	}
							...
							{this.state.iCanHazTable === true ?
								this.state.hasTable === true ?
									<div className="table-container">
										...
										<input className="table-button" type="submit" value="Edit" onClick={this.editTable}/>
									</div>
								:
									<div className="table-container">
