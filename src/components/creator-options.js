...
export default class Options extends React.Component {
	...
	handleRandomizationChange(event) {
		...
		if (this.props.qset.randomization > 0) {
			for (let i = 0; i < this.props.qset.dimensions.x; i++) {
				for (let j = 0; j < this.props.qset.dimensions.y; j++) {
					this.setState(Object.assign(this.props.qset.items[0].items[i][j].options, { blank: false }))
				}
			}
		}
	}
		...
		return (
			<div className="options-bar">
				...
				<div className="instructions">
					<div>
						<svg tabIndex={0} onClick={this.props.toggleInstruction} onKeyDown={(e) => {if (e.key === 'Enter') {this.props.toggleInstruction()}}} xmlns="http://www.w3.org/2000/svg" version="1.0" width="34" height="34" viewBox="0 0 344.000000 344.000000">
